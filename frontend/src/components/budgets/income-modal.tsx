...
import { CreateIncomeRequest, UpdateIncomeRequest, Frequency, Income } from '@/types/budget'
...
export const IncomeModal: React.FC<IncomeModalProps> = ({
    ...
    if (isOpen) {
      if (incomeToEdit) {
        setFormData({
          ...
          is_net: incomeToEdit.is_net,
          effective_month: incomeToEdit.effective_month,
          effective_year: incomeToEdit.effective_year,
        })
      } else {
        setFormData({
          description: '',
  ...
  }, [isOpen, initialFrequency, incomeToEdit])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && incomeToEdit) {
        ...
        await incomeService.updateIncome(incomeToEdit.id, updateData)
      } else {
      ...
      setError(err.message || `Failed to ${isEditing ? 'update' : 'create'} income entry`)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEditing ? 'Edit Income' : 'Add Income Source'}>
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex items-center justify-between p-4 rounded-xl bg-secondary/50">
          <div>
            <p className="text-sm font-semibold text-primary">Net Income</p>
            <p className="text-xs text-muted mt-1">After tax and deductions</p>
          </div>
          <button
            type="button"
            onClick={() => setFormData({ ...formData, is_net: !formData.is_net })}
            className={`relative w-12 h-6 rounded-full transition-all duration-300 ${
              formData.is_net ? 'bg-primary' : 'bg-primary/30'
            }`}
          >
            <div
              className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all duration-300 ${
                formData.is_net ? 'left-7' : 'left-1'
              }`}
            />
          </button>
        </div>
